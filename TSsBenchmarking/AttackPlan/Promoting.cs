...
{
    public class WhiteWashing<H, Rec, Rcf> : IPlan<H>
        where H : IHistory, new()
        where Rcf : IRecommendationFunction<H, Rec>, new()
    {
        public WhiteWashing(TruSyFire.TrustSystems.Environment.Environment e)
            : base(e) { }
        public override string Title
        {
            get { return "WhiteWashing"; }
        }
        ...
        public override ClientAtomicAction GetAction(TruSyFire.POMDP.Belief<TruSyFire.Verification.State<H>> Belief, MaliciousClient attacker)
        {
            if (Recs == null)
            {
                ...
                foreach (var sp in Environment.HonestSP)
                    Recs[sp] = rcf.WorstRecommend;
                foreach (var sp in Environment.MaliciousSP)
                    Recs[sp] = rcf.BestRecommend;
            }
